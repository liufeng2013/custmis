...
public class Pager {
	...
	void beginRender(MarkupWriter writer) {
		...
		if (maxPages < 2)
			return;
		writer.element(resources.getContainer().getComponentResources()
				.getElementName(), "class", "ck_paged_loop_pager");
	...
	void onAction(int newPage) {
		...
		if (forId != null) {
			getPagedLoopComponent().getComponentResources().triggerEvent(
					EventConstants.ACTION, new Integer[] { newPage }, null);
		}
	}

	private Component getPagedLoopComponent() {
		if (forId != null && pagedLoopComponent == null)
			pagedLoopComponent = resources.getPage().getComponentResources()
					.getEmbeddedComponent(forId);
