AjaxValidator = Class.create({
	initialize : function(spec) {
		...
		// Set up a listener that validates the field - asynchronously in the
		// server - on change of focus
	...
	asyncValidateInServer : function() {
		...
		if (value) {
			listenerURIWithValue = addRequestParameter('param', value,
					this.listenerURI);
			new Ajax.Request(listenerURIWithValue, {
						...
						onSuccess : function(t) {
							if (t.responseJSON.error) {
								this.field
										.showValidationMessage(t.responseJSON.error);
							}
						}.bind(this)
					});
		}
...
// Extend the Tapestry.Initializer with a static method that instantiates an
// AjaxValidator.
